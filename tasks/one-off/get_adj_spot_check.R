...
get_adj_spot_check <- function(){
  ...
  coders <- tribble(
    ...
    50, "adj_david", "2022-09-12", "2022-10-12",
  ...
  updated_canonical_events <- tbl(con, "canonical_event") |>
    collect() |>
    select(canonical_id = id, key, description) |>
    ...
    janitor::clean_names() |>
    select(canonical_id, key, description, start_date,
    ...
    select(canonical_id, key, description, editor,
