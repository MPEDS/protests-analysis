...
```{r}
racial_issues <- mpeds |> 
  select(canonical_id, key, start_date, racial_issue) |> 
  st_drop_geometry() |> 
  unnest(cols = c(racial_issue)) |> 
...
```{r}
  ...
  mizzou_wave <- get_any_umbrella("Umbrella_Mizzou_Anti-Racism_2015-2016_Oct-Feb")
...
us_protests_issues <- us_protests |> 
 ...
 third_us_wave_summary <- tibble(
   data_frame = c("trump_not_immigration", "immigration_not_trump", "trump_and_immigration"),
   row_count = third_us_wave_counts 
   ) 
 
 third_us_wave_summary <- third_us_wave_summary |>
   add_row(data_frame = "total", row_count = sum(third_us_wave_counts)) 

 make_weekly_count <- function(issue_df) {
   weekly_count <- issue_df |> 
   mutate(week_start = as.Date(format(start_date - (as.numeric(format(start_date, "%u")) - 1), "%Y-%m-%d"))) |> 
   group_by(week_start) |> 
   summarize(n = n())
 }
...
#   writexl::write_xlsx("docs/trends_paper/us_issues.xlsx")
# 
# get_canada_issue_counts(mpeds) |> 
#   writexl::write_xlsx("docs/trends_paper/canada_issues.xlsx")
