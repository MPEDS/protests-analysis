...
ggplot(protests_by_country, aes(x = date, y = n)) + 
  geom_bar(stat = "identity") + 
  facet_wrap(factor(protests_by_country$country, levels=c("US", "Canada")), scales = "free_y", 
             axes = "all", axis.labels = "all", ncol = 1) + 
...
```{r Figure2-us_over_time, width = 12, height = 8}
...
```{r Figure3-canada_over_time, width = 12, height = 8}
make_plot("Canada")
```
...
bind_cols(university_counts, location_counts) |> 
  kable(col.names = c("University", "Count", "City", "Count"))
```
...
make_issue_counts_by_year <- function(country_opt, sub_dta = NULL) {
  ...
  dates <- country |>
    ...
    group_by(year) |>
    summarise("year_n" = sum(length(unique(key))))
  ...
  new_count_attempt <- issues_values |> 
    filter(str_detect(value, "University governance")) |> 
    group_by(key) |> 
    filter(n() < 2) |> 
    group_by(value) |> 
    summarize(n=n()) |> 
    mutate(name = "issue OR racial_issue")
  ...
  top_issues <- issues_values |>
    ...
    select(!lst) |> 
    bind_rows(new_count_attempt)

  issue_count_by_year <- issues_values |>
    right_join(top_issues, by = c("name", "value")) |>
    group_by(year, issue = value, type = name, issue_n_all_years = n) |>
    summarize(n = n())  |>
    left_join(dates, by ="year") |> 
    mutate(n = ifelse(
      is.na(n), "", paste0(
        round(100 * n/year_n, 2), "% (", format(n, big.mark = ",") |> 
          str_trim(), ")")
      )
    ) |> 
    ungroup() |>
...
make_issue_counts_by_semester <- function(country_opt, sub_dta = NULL) {
  if(is.null(sub_dta)){
    sub_dta <- mpeds
  }
  ...
  dates <- country |>
    ...
    filter(!str_detect(start_date, "2011")|!str_detect(end_date, "2011")) |>
    mutate(year = year(start_date),
           year_half = ifelse(month(start_date) < 7, "a","b")) |>
    group_by(year, year_half) |> 
    summarise("year_n" = sum(length(unique(key))))
  ...
  issues_values <- country |>
    select(key, issue, racial_issue, start_date) |>
    pivot_longer(cols = c(everything(), -key, -start_date)) |>
    unnest(value) |>
    filter(value != "_Not relevant", value != "",
           value != "Anti-racism")
  ...
  issue_count_by_semester <- issues_values |>
    bind_rows(new_count_attempt) |> 
    right_join(top_issues, by = c("name", "value")) |> 
    mutate(year_half = ifelse(month(start_date) < 7, "a", "b"),
    ...
    group_by(year,
             ...
             issue_n_all_years = n) |> 
    summarize(n = n()) |>    
    left_join(dates, by =c("year", "year_half")) |>   
    mutate(n = ifelse(
      is.na(n), "", paste0(
        round(100 * n/year_n, 2), "% (", format(n, big.mark = ",") |> 
          str_trim(), ")")
    ...
    ungroup() |>
    mutate(year = paste0(year, year_half)) |> 
    select(-year_n, -year_half) |> 
    pivot_wider(names_from = "year",
                values_from = "n") |>
    select(-contains("2011")) |>
    arrange(desc(total_n))
  
  # ADD fix to univ governance row
  
  
  return(issue_count_by_semester)
}
  ...
  make_issue_counts_by_semester("US") |> 
    kable()
...
```{r, message=FALSE}
 make_issue_counts_by_semester("Canada") |> 
    kable()
...
us_protests_issues <- us_protests |>  
  select(canonical_id, key, description, start_date, issue, racial_issue) |> 
  unnest(issue) |> 
  unnest(racial_issue)

# hard codes the specific parameters for getting this US wave
# in this case:
#   - racial_issue == "Police violence"
#   - dates between "2014-11-20" and "2014-12-15"
get_first_us_wave <- function() {
  
  first_us_wave <- us_protests_issues |> 
    select(-issue) |> 
    filter(racial_issue == "Police violence") |> 
    filter(start_date >= as.Date("2014-11-20") & 
           start_date <= as.Date("2014-12-15"))
  
  return(first_us_wave)
}

first_us_wave <- get_first_us_wave()


# creates a data frame with a week-by-week count of the number of protests for 
# a given wave data frame of filtered canonical events from integrated
make_weekly_count <- function(wave_df) {
  
  # get the bounds of the wave
  first_protest_date <- min(wave_df$start_date)
  last_protest_date <- max(wave_df$start_date)
 ...
 # create a summary data frame of the wave, with basic counts
 # this is hard coded for each wave 
 first_us_wave_summary <- tibble(
   ...
   n_protests = nrow(first_us_wave)
   )
   
 first_us_wave_weekly_count <- make_weekly_count(first_us_wave)
 
 writexl::write_xlsx(lst(first_us_wave_summary, 
                         first_us_wave_weekly_count),             "docs/trends-paper/wave-counts/us-wave-counts/first_us_wave_counts.xlsx")
 ...
 first_us_wave_daily_count <- make_daily_count(first_us_wave)
...
make_wave_line_graph_weekly <- function(daily_count_df,graph_title) {

  ggplot(data = daily_count_df, aes(x = week_start, y = n)) +
    geom_bar(stat = "identity") +
    expand_limits(y = 0) +
    labs(x = "Date", y = "Protest Count", title = graph_title) +
    theme(
  ...
  make_wave_line_graph(first_us_wave_daily_count,"First US Wave (BLM)") +
...
```{r Mizzou}
# using two different sets of criteria for the mizzou wave
#     1. canonical events under the mizzou umbrella
#     2. filtering by racial_issue == "University governance..." or 
#                     racial_issue == "Campus climate"
  
  # 1. mizzou umbrella
  source("tasks/one-off/get_any_umbrella.R") #helper function
  
  mizzou_umbrella_wave <- get_any_umbrella("Umbrella_Mizzou_Anti-Racism_2015-2016_Oct-Feb") |>
  filter(start_date >= as.Date("2015-10-01") & 
         start_date <= as.Date("2015-12-31"))
  
  mizzou_umbrella_weekly_count <- make_weekly_count(mizzou_umbrella_wave)
  mizzou_umbrella_daily_count <- make_daily_count(mizzou_umbrella_wave)
  
  # 2. mizzou by issue
  get_second_us_wave <- function() {
      ...
      filter(start_date >= as.Date("2015-10-01") & 
             start_date <= as.Date("2015-12-31"))
      return(univ_admin_and_campus_climate) 
  }
  
  mizzou_by_issue_wave <- get_second_us_wave()
  
  mizzou_by_issue_weekly_count <- make_weekly_count(mizzou_by_issue_wave)
  mizzou_by_issue_daily_count <- make_daily_count(mizzou_by_issue_wave)
  
  # make the summary, including counts from the two different sets of criteria
  second_us_wave_summary <- tibble(
   wave = "Second US Wave (Mizzou)",
   criteria = c("Mizzou umbrella", "racial_issue == Univ governance... OR racial_issue == Campus climate"),
   n_protests = c(nrow(mizzou_umbrella_wave), nrow(mizzou_by_issue_wave))
  )
  
  writexl::write_xlsx(lst(second_us_wave_summary, 
                         mizzou_by_issue_weekly_count),
                      "docs/trends-paper/wave-counts/us-wave-counts/second_us_wave_counts.xlsx")

  make_wave_line_graph(mizzou_umbrella_daily_count, "Second US Wave (Mizzou umbrella Oct 1-Dec 31)") +
    scale_x_date(date_breaks = "2 days",
                 date_labels = "%b %d",
                 expand = c(0, 0)) +
    scale_y_continuous(expand = c(0, 0), breaks = seq(0, 50, by = 5), limits = c(0, 40))
  
```

```{r Mizzou-by-issue}

  make_wave_line_graph(mizzou_by_issue_daily_count, "Second US Wave (Mizzou by issue)") +
        scale_x_date(date_breaks = "2 days",
                 date_labels = "%b %d",
                 expand = c(0, 0)) +
        scale_y_continuous(expand = c(0, 0), breaks = seq(0,100, by = 5), limits = c(0,70))
```

```{r}

  mizzou_campaign_wave <- mizzou_umbrella_wave |> 
    filter(relationship_type == "campaign")

  mizzou_campaign_daily_count <- make_daily_count(mizzou_campaign_wave)

  make_wave_line_graph(mizzou_campaign_daily_count, "Second US Wave (Mizzou campaign)") +
        scale_x_date(date_breaks = "2 days",
                 date_labels = "%b %d",
                 expand = c(0, 0)) +
...
```{r ThirdWave}
 ...
 third_us_wave_summary <- tibble(
   wave = "Third US Wave (Trump)",
   criteria = c("trump_not_immigration", "immigration_not_trump", "trump_and_immigration"),
 ...
 trump_not_immigration_daily_count <- make_daily_count(trump_not_immigration)
 immigration_not_trump_daily_count <- make_daily_count(immigration_not_trump)
 trump_and_immigration_daily_count <- make_daily_count(trump_and_immigration)
 ...
 colors <- c("Trump not immigration" = "red", "Immigration not Trump" = "blue", "Trump and immigration" = "black")

make_third_wave_stacked_bar_graph <- function() {
    ggplot() +
    geom_bar(data = trump_not_immigration_weekly_count, aes(x = week_start, y = n, fill = "Trump not immigration"), 
             stat = "identity", position = "stack") +
    geom_bar(data = immigration_not_trump_weekly_count, aes(x = week_start, y = n, fill = "Immigration not Trump"), 
             stat = "identity", position = "stack") +
    geom_bar(data = trump_and_immigration_weekly_count, aes(x = week_start, y = n, fill = "Trump and immigration"), 
             stat = "identity", position = "stack") +
...
make_third_wave_stacked_bar_graph()

```

```{r ThirdWave-ScatterPlot}

make_third_wave_scatterplot <- function() {
    ggplot() +
    geom_point(data = trump_not_immigration_daily_count, aes(x = start_date, y = n, color = "Trump not immigration"), size = 1.2) +
    geom_point(data = immigration_not_trump_daily_count, aes(x = start_date, y = n, color = "Immigration not Trump"), size = 1.2) +
    geom_point(data = trump_and_immigration_daily_count, aes(x = start_date, y = n, color = "Trump and immigration"), size = 1.2) +
    scale_x_date(expand = c(0, 0),date_breaks = "3 days", date_labels = "%b %d") +
    labs(x = "Date", y = "Protest Count", title = "Third US Wave") +
    scale_y_continuous(expand = c(0, 0), breaks = seq(0, 100, by = 5), limits = c(0,65)) +
    theme(
      axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1, size = 8),
    legend.title = element_blank()
  ) +
...
make_third_wave_scatterplot()

```

## Third Wave - Trump / Sanctuary
```{r ThirdWave-Trump-ALH}
us_protests_issues |>
  mutate(is_trump = str_detect(issue, "Trump and/or his administration \\(Against\\)"),
         is_immigration = str_detect(racial_issue, "Immigration \\(For\\)"),
         is_sanctuary = str_detect(description, regex("sanctuary", ignore_case = TRUE))
         )|>
  filter(start_date >= '2016-11-01' & start_date <=  '2017-02-28',
         is_trump == 1 | is_immigration == 1) |>
  mutate(protest_type =
  ...
  ggplot(aes(x = start_date, fill = protest_type)) +
    ...
    scale_fill_manual(values = c("Sanctuary" = "blue2",
...
```{r}
#make_third_wave_line_graph() + make_third_wave_scatterplot()
```
...
us_waves_summary <- tibble(
  wave = c("First Wave", "Second Wave (Mizzou umbrella)", "Third Wave (trump AND immigration count)"),
  n_protests = c(nrow(first_us_wave), nrow(mizzou_umbrella_wave), nrow(trump_and_immigration))
)
...
```{r}
  ...
  quebec_tuition_daily_count <- make_daily_count(quebec_tuition_protests)
  quebec_police_violence_daily_count <- make_daily_count(quebec_police_violence_protests)

  first_canada_wave_summary <- tibble(
     wave = "First Canada Wave (Quebec 2012)",
     criteria = c("year = 2012 AND location = QC AND issue = \"Tuition\"", "Quebec Police Violence Protests in 2012"),
  ...
  first_canada_wave_summary |> 
    kable()
```

```{r}
  canada_first_wave_colors <- c("Tuition" = "aquamarine4", "Police Violence" = "brown")

make_canada_first_wave_graph <- function() {
    ggplot() +
    geom_bar(data = quebec_tuition_weekly_count, aes(x = week_start, y = n, fill = "Tuition"), 
             stat = "identity", position = "stack") +
    geom_bar(data = quebec_police_violence_protests_weekly_count, aes(x = week_start, y = n, fill = "Police Violence"), 
             stat = "identity", position = "stack") +
    scale_x_date(expand = c(0, 0), date_breaks = "1 weeks", date_labels = "%b %d") +
    labs(x = "Date", y = "Protest Count", title = "First Canada Wave") +
    scale_y_continuous(expand = c(0, 0), limits = c(0, 11), breaks = seq(0, 100, by = 1)) +
    theme(
        axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1, size = 8),
        legend.title = element_blank()
    ) +
    scale_fill_manual(values = canada_first_wave_colors)
}
  
  
  make_canada_first_wave_graph()
```
...
```{r}
  ...
  toronto_labor_daily_count <- make_daily_count(toronto_labor_protests)
  ...
  quebec_labor_daily_count <- make_daily_count(quebec_labor_protests)
  
  quebec_economy_protests <- quebec_protests |> 
      filter(issue == "Economy/inequality") |> 
      filter(str_detect(start_date, "2015-") | str_detect(start_date, "2014-"))
    
  quebec_economy_daily_count <- make_daily_count(quebec_economy_protests)
  quebec_economy_weekly_count <- make_weekly_count(quebec_economy_protests)

  second_canada_wave_summary <- tibble(
     wave = "Second Canada Wave (Labor)",
     criteria = c("Toronto Labor Protests 2014/2015", 
                  "Quebec Labor Protests 2014/2015", 
                  "Quebec Economy/inequality Protests 2014/2015"),
     n_protests = c(nrow(toronto_labor_protests), 
                    nrow(quebec_labor_protests), 
                    nrow(quebec_economy_protests)) 
     )
...
```{r}
  ...
  canada_second_wave_colors <- c("Toronto Labor" = "coral4", "Quebec Labor" = "darkcyan", "Quebec Economy" = "black")

first_date <- min(c(min(toronto_labor_weekly_count$week_start),
                    min(quebec_labor_weekly_count$week_start),
                    min(quebec_economy_weekly_count$week_start)))
    
last_date <- max(c(max(toronto_labor_weekly_count$week_start),
                   max(quebec_labor_weekly_count$week_start),
                   max(quebec_economy_weekly_count$week_start)))
  
make_canada_second_wave_graph <- function() {
    ggplot() +
    geom_bar(data = toronto_labor_weekly_count, aes(x = week_start, y = n, fill = "Toronto Labor"), 
             stat = "identity", position = "stack") +
    geom_bar(data = quebec_labor_weekly_count, aes(x = week_start, y = n, fill = "Quebec Labor"), 
             stat = "identity", position = "stack") +
    geom_bar(data = quebec_economy_weekly_count, aes(x = week_start, y = n, fill = "Quebec Economy"), 
             stat = "identity", position = "stack") +
    scale_x_date(expand = c(0, 0), 
                 limits = c(first_date, last_date), 
                 breaks = seq(from = first_date, to = last_date, by = "2 week"),
                 date_labels = "%b %d") +
    labs(x = "Date", y = "Protest Count", title = "Second Canada Wave") +
    scale_y_continuous(expand = c(0, 0), limits = c(0, 7), breaks = seq(0, 100, by = 1)) +
    theme(
        axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1, size = 8),
        legend.title = element_blank()
    ) +
    scale_fill_manual(values = canada_second_wave_colors)
}
  
  
  make_canada_second_wave_graph()
```
...
canada_waves_summary <- tibble(
  wave = c("First Wave", "First Wave", "Second Wave", "Second Wave","Second Wave"),
  criteria = c("year = 2012 AND location = QC AND issue = \"Tuition\"", 
               "Quebec Police Violence Protests in 2012",
               "Toronto Labor Protests 2014/2015", 
               "Quebec Labor Protests 2014/2015", 
               "Quebec Economy/inequality Protests 2014/2015"),
  n_protests = c(nrow(quebec_tuition_protests), 
                 nrow(quebec_police_violence_protests),
                 nrow(toronto_labor_protests), 
                 nrow(quebec_labor_protests), 
                 nrow(quebec_economy_protests))
)

canada_waves_summary |> 
  kable()
```
