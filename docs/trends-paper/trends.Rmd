...
blm_wave <- us_protests_issues |> 
 ...
 make_weekly_count <- function(issue_df) {
   first_protest_date <- min(issue_df$start_date)
   last_protest_date <- max(issue_df$start_date)
   all_weeks <- seq(first_protest_date, last_protest_date, by = "week")
...
```{r}
  ...
  quebec_tuition_protests <- canada_protests_issues |>
  ...
  first_canada_wave_summary <- tibble(
     wave = "Quebec Tuition Protests in 2012",
     n_protests = nrow(quebec_tuition_protests) 
     )
