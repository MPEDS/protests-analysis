...
top_issues <- mpeds |> 
  ...
  slice_head(n = 7) |> 
  pull(issue) 
issue_crosstab <- function(colname){
  mpeds |> 
    select({{colname}}, issue) |> 
    unnest(issue) |> 
    unnest({{colname}}) |> 
    filter(issue %in% top_issues) |> 
    group_by(issue, {{colname}}) |> 
    count() |> 
    kable()
}

issue_crosstab(police_presence_and_size)
issue_crosstab(police_activities)
issue_crosstab(type_of_police)
issue_crosstab(university_action_on_issue)
issue_crosstab(university_discourse_on_issue)
issue_crosstab(university_reactions_to_protest)
```
...
issue_crosstab_over_time <- function(colname){
  agg <- mpeds |> 
    select({{colname}}, start_date, issue) |> 
    mutate(date = floor_date(start_date, "months")) |> 
    unnest(issue) |> 
    unnest({{colname}}) |> 
    filter(issue %in% top_issues) |> 
    group_by(date, issue, {{colname}}) |> 
    count()
  ...
  ggplot(agg, aes(x = date, y = n, fill = {{colname}})) + 
    geom_bar(stat = "identity") + 
    facet_wrap(~issue) + 
    labs(x = "Date (floored to months",
         y = "Count of appearances",
         title = paste0(name_string, " by top issues over time.")) + 
    theme(text = element_text(size = 16))
}

issue_crosstab_over_time(police_presence_and_size)
issue_crosstab_over_time(police_activities)
issue_crosstab_over_time(type_of_police)
issue_crosstab_over_time(university_action_on_issue)
issue_crosstab_over_time(university_discourse_on_issue)
issue_crosstab_over_time(university_reactions_to_protest)
```
