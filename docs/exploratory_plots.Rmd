...
protests_by_country <- mpeds |> 
  mutate(
    country = ifelse(
      is.na(canada_metropolitan_area), "Canada", "United States"
    )
    ) |> 
  mutate(date = floor_date(start_date, "month")) |> 
...
mizzou_sf <- mizzou_events |> 
  ...
  count() |> 
  ungroup() |> 
  left_join(county_sf, by = "fips") |> 
  mutate(geometry = st_centroid(geometry)) |> 
  st_as_sf(crs = st_crs(us_sf)) 
...
top_mizzou <- mizzou_sf |> 
  drop_na() |> 
  arrange(desc(n)) |> 
  slice_head(n = 5) |> 
  left_join(county_labels, by = "fips")

ggplot(mizzou_sf) +
  geom_sf(data = us_sf) +
  geom_sf(data = state_sf, color = "gray") +
  geom_sf(aes(size = n), alpha = 0.5) + 
  geom_sf_label(data = top_mizzou,
          aes(label = name),
          nudge_x = -3,
          nudge_y = 3) +
  labs(
...
quebec_events <- canonical_event_relationship |> 
  filter(canonical_id2 %in% c(5558, 2939, 6396, 2551)) 
