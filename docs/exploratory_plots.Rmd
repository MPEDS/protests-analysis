...
```{r police_counts}
glimpse_counts <- function(colname, dta = mpeds, percent = FALSE, make_kable = TRUE){
  total_events <- dta |> 
    filter(!str_detect(key, "^Umbrella")) |> 
    select(key) |> 
    distinct() |> 
  ...
  dta <- dta |>
    ...
    count({{ colname }}) |> 
    arrange(desc(n)) |> 
    mutate(n = ifelse(percent,
                      paste0(round(100 * n / total_events, 2), "%"),
                      n)) 
  if(make_kable){
    return(kable(dta))
  } else {
...
combined <- mpeds |> 
  select(key, geoid, issue, racial_issue) |> 
  pivot_longer(cols = c(issue, racial_issue)) |> 
  select(-name) |> 
  unnest(value) |> 
...
us_issues <- glimpse_counts(issue, filter(combined, country == "US"), TRUE, FALSE) |> 
  rename(US = n)
canada_issues <- glimpse_counts(issue, filter(combined, country == "Canada"), TRUE, FALSE) |> 
  rename(Canada = n)

left_join(us_issues, canada_issues, by = "issue") |> 
  kable()
```
...
```{r police_issue_baseline, width = 10, height = 8}
...
make_line_plot <- function(dta){
  ...
  geom_vline(linetype = "dashed", 
             xintercept = as.Date("2012-02-01"), color = events_color) +
  annotate(geom = "label", label = "Quebec\nprotests",
           color = events_color, x = as.Date("2012-06-01"), y = 300,
           family = "Lato") + 
  
  geom_vline(linetype = "dashed", 
             xintercept = as.Date("2014-11-01"), color = events_color) +
  annotate(geom = "label", label = "Darren Wilson\nnon-indictment",
           color = events_color, x = as.Date("2014-05-15"), y = 275,
           family = "Lato") + 
  
  geom_vline(linetype = "dashed",
             xintercept = as.Date("2015-10-01"), color = events_color) +
  annotate(geom = "label", label = "Mizzou\nprotests",
           color = events_color, x = as.Date("2016-01-15"), y = 325,
           family = "Lato") + 
  
  geom_vline(linetype = "dashed", 
             xintercept = as.Date("2016-11-01"),
             color = events_color) +
  annotate(geom = "label", label = "Anti-Trump\nprotests",
           color = events_color, x = as.Date("2017-04-01"), y = 300,
           family = "Lato") + 
