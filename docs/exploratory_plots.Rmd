...
mpeds |> 
  mutate(date = floor_date(start_date, "year")) |> 
  group_by(date) |> 
  count() |> 
  ggplot(aes(x = date, y = n)) + 
  geom_line() +
  labs(
    title = "Same thing, floored to years"
  )
...
plot_uni_counts <- function(unit){
  unis_over_time <- mpeds |> 
    mutate(date = floor_date(start_date, unit = unit)) |> 
  ...
  uni_plot <- ggplot(unis_over_time, aes(x = date, y = n, color = university)) + 
    geom_line() + 
    labs(
      x = paste0("Time (", unit, ")"),
      y = "Number of associated canonical events",
      ...
      "Dates are aggregated to the", unit, "unit.")
    )
  ...
  uni_grouped_plot <- ggplot(uni_grouping_over_time, aes(x = date, y = n, color = group)) + 
    geom_line() + 
    labs(
      x = paste0("Time (", unit, ")"),
      y = "Number of associated canonical events",
      title = "Top university groups by protest count over time",
      caption = paste("Dates are aggregated to the", unit, "unit.")
    ) 
  
  return(list(uni_plot, uni_grouped_plot))
}

c("day", "week", "14 days", "month", "6 months", "year") |> map(plot_uni_counts)
...
issues |> 
  ...
  group_by(issue, date) |> 
  count() |> 
  ggplot(aes(x = date, y = n, color = issue)) + 
  geom_line() + 
  labs(
    y = "Number of associated canonical events",
    x = "Date (month)"
...
# I use `n_eventS[1]` to resolve that
issues |> 
  group_by(date) |> 
  mutate(
    n_events = n()
