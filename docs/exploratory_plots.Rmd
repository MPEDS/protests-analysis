...
n_universities <- mpeds$university |>
  bind_rows()  |>
...
# counts for universities -- defined as university where protest took place
# which is "other univ where protest took place" if present and publication otherwise
# So to count missings, look for rows where neither are present
missing_uni_locations <- mpeds |> 
  filter(!str_detect(key, "Umbrella")) |> 
  mutate(uni_missing = map_lgl(university, ~!any(.$uni_name_source %in% c(
    "publication", "other univ where protest occurs"
  ))), uni_location_missing = map_lgl(university, ~all(is.na(.$lat))))

n_missing_unis <- sum(missing_uni_locations$uni_missing)
n_missing_uni_locations <- sum(missing_uni_locations$uni_location_missing)
...
issue_count <- mpeds |> 
  ...
  distinct() |> 
  count(key)
n_one_both_issue <- issue_count |> 
  filter(n >= 2) |> 
  nrow()
n_one_issue <- issue_count |> 
  filter(n >= 1) |> 
  nrow()
...
tribble(
  ...
  "Total non-umbrella events", mpeds_nonumbrella,
  "Unique locations", n_locations,
  "US counties", n_counties,
  "Canadian CMAs", n_cmas,
  "Universities", n_universities,
  "CEs with missing universities", n_missing_unis,
  "Universities with missing locations", n_missing_uni_locations,
  ...
  "# of events with at least one issue or racial issue", n_one_issue,
  "# of events with at least one issue and racial issue", n_one_both_issue,
...
```{r police_counts}
glimpse_counts <- function(colname, dta = mpeds, percent = FALSE, make_kable = TRUE, public_private = FALSE){
  total_events <- dta |> 
    filter(!str_detect(key, "^Umbrella")) |> 
    select(key) |> 
    distinct() |> 
  ...
  dta_cleaned <- dta |>
    ...
    select(country, {{colname}}) |> 
    unnest(cols = {{colname}}) 
  
  dta_display <- dta_cleaned |> 
    group_by(country, {{ colname }}) |> 
    count() |> 
    drop_na(country) |> 
    pivot_wider(names_from = country, values_from = n) |> 
    mutate(Total = US + Canada) |> 
    arrange(desc(Total)) |> 
    mutate(across(c(Total, US, Canada), ~ifelse(percent,
                      paste0(round(100 * . / total_events, 2), "%"),
                      .)))
  ...
  if(public_private){
    dta_cleaned <- dta |>
      ...
      select(university, country, {{colname}}) |> 
      unnest(cols = {{colname}}) 
    dta_us <- dta_cleaned |> 
      filter(country == "US") |> 
      mutate(is_public = if_else(map_lgl(university, ~.$is_uni_public[1]), "Public", "Private")) |> 
      group_by(is_public, {{ colname }}) |> 
      count() |> 
      pivot_wider(names_from = is_public, values_from = n) |> 
      arrange(desc(Public)) |> 
      mutate(across(c(Public, Private), ~ifelse(percent,
                      paste0(round(100 * . / total_events, 2), "%"),
                      .)))
    if(make_kable){
      return(kable(dta_us))
...
```{r}
glimpse_counts(police_presence_and_size, public_private = TRUE)
glimpse_counts(police_activities, public_private = TRUE)
glimpse_counts(type_of_police, public_private = TRUE)
```
...
Issue counts here were combined for each canonical event, such that 
no issue, racial issue, or some combination of the two is counted twice.
If an event has "University governance" for both issue and racial issue, 
it is counted once.

The table below thus represents the proportion of all events a given combined
racial-nonracial issue was relevant to.

```{r}
# combining racial and non-racial into one list-col
combined <- mpeds |> 
  select(key, country, geoid, issue, racial_issue) |> 
  pivot_longer(cols = c(issue, racial_issue)) |> 
  unnest(value) |> 
  mutate(value = if_else(name == "racial_issue", paste0(value, " (racialized)"), value)) |> 
  select(-name) |> 
  filter(!str_detect(value, "_Not relevant"), value != "") |> 
  distinct() |> 
  group_by(key, country, geoid) |> 
  summarize(issue = list(value), .groups = "drop")

glimpse_counts(issue, combined, TRUE)
...
```{r police_involvement_by_issue, fig.height = 10, fig.width = 10}
police_involvement <- mpeds |> 
  filter(
    if_any(c(type_of_police, police_actions_text,
...
# GAH can't think of a neater way to do this
extreme_police_issue <- mpeds |> 
  filter(
    (map_lgl(police_activities, ~"Force: 2+ Weapon Types" %in% .) + 
    map_lgl(police_activities, ~"Arrest- Large Scale" %in% .) + 
    map_lgl(police_presence_and_size, ~"Heavily Policed" %in% .) +
    map_lgl(police_presence_and_size, ~"Motorized Presence" %in% .) +
    map_lgl(police_presence_and_size, ~"Substantial" %in% .) >= 2) |
    map_lgl(type_of_police, ~'"Riot police"' %in% .)
  ) |>  
  get_issue() |> 
  arrange(desc(n))
...
# x <- mpeds |> 
#   mutate(
#     is_weapon = map_lgl(police_activities, ~"Force: 2+ Weapon Types" %in% .),
#     is_large = map_lgl(police_activities, ~"Arrest- Large Scale" %in% .),
#     is_heavy = map_lgl(police_presence_and_size, ~"Heavily Policed" %in% .),
#     is_motorized = map_lgl(police_presence_and_size, ~"Motorized Presence" %in% .),
#     is_substantial = map_lgl(police_presence_and_size, ~"Substantial" %in% .),
#     is_riot = map_lgl(type_of_police, ~'"Riot police"' %in% .)
#   ) |> 
...
plot_comparison <- function(dta, name){
  dta <- list("All protests" = mpeds_police_issue,
    ...
    mutate(type = ifelse(type == "category",
                         ...
                         type)) |> 
    filter(value %in% dta$value) |> # value = issue name
  ...
  dta |> 
    ...
    labs(
      title = paste0("Top issue occurrences in general protests and ", name),
...
# All, police-involved, extreme policing on one graph
police_issue_combined <- list("All protests" = mpeds_police_issue,
     "Police-involved protests" = police_involvement_issue,
     "Protests with extreme policing" = extreme_police_issue) |> 
  bind_rows(.id = "type") |> 
  drop_na(value)

# ARGH
police_issue_diffs <- police_issue_combined |> 
  pivot_wider(names_from = type,
               values_from = n) |> 
  pivot_longer(cols = c("Police-involved protests", "Protests with extreme policing"),
              values_to = "pct") |> 
  mutate(diff_value = round(pct - `All protests`, 2),
         diff = ifelse(diff_value > 0, paste0("+", diff_value), as.character(diff_value)))  |> 
  select(type = name, value, all_protests = `All protests`, diff_value, diff)

police_issue_combined |> 
  filter(value %in% top_extreme_police_issue$value) |> # value = issue name
  left_join(police_issue_diffs, by = c("value", "type")) |> 
  mutate(value = str_wrap(value, 13, whitespace_only = FALSE),
         value = factor(value, levels = str_wrap(extreme_police_issue$value, 13, whitespace_only = FALSE))) |> 
  ggplot(aes(x = value, fill = value, y = n)) + 
  geom_bar(stat = "identity") + 
  geom_text(aes(label = diff), vjust = -0.5) +
  facet_wrap(~type, nrow = 3) + 
  labs(
    title = "Top issue occurrences in across police involvement levels",
    y = "Percentage of protests associated with issue",
    x = NULL,
    caption = paste0(
      "Top issues are defined as the seven most common issues for protests with extreme policing.\n",
      "Difference from corresponding measure for all protests are labeled above bars."),
  ) + 
  scale_fill_brewer(type = "qual", palette = "Set1") + 
...
```{r police_issue_separate}
# Top across extreme police
top_extreme_police_issue |> 
  left_join(filter(police_issue_diffs, type == "Protests with extreme policing"),
            by = "value") |> 
  mutate(value = str_wrap(value, 13, whitespace_only = FALSE),
         value = factor(value, levels = str_wrap(top_extreme_police_issue$value, 13, 
                                                 whitespace_only = FALSE))) |> 
  ggplot(aes(x = value, fill = value, y = n)) + 
  geom_bar(stat = "identity") + 
  geom_text(aes(label = diff), vjust = -0.5) +
  labs(
    title = "Top issue occurrences in protests with extreme policing",
    y = "Percentage of protests associated with issue",
    x = NULL,
    caption = paste0(
      "Top issues are defined as the eight most common issues for protests with extreme policing.\n",
      "Difference from corresponding measure for all protests are labeled above bars."),
  ) + 
  scale_fill_brewer(type = "qual", palette = "Set1") + 
  scale_y_continuous(breaks = seq(0, 50, by = 5), limits = c(0, 50)) + 
...
top_police_issues |> 
  left_join(filter(police_issue_diffs, type == "Police-involved protests"),
            by = "value") |> 
  mutate(value = str_wrap(value, 13, whitespace_only = FALSE),
         value = factor(value, levels = str_wrap(top_police_issues$value, 13, 
                                                 whitespace_only = FALSE))) |> 
  ggplot(aes(x = value, fill = value, y = n)) + 
  geom_bar(stat = "identity") + 
  geom_text(aes(label = diff), vjust = -0.5) +
  labs(
    title = "Top issue occurrences in police-involved protests",
    y = "Percentage of protests associated with issue",
    x = NULL,
    caption = paste0(
      "Top issues are defined as the eight most common issues for protests with police involvement.\n",
      "Difference from corresponding measure for all protests are labeled above bars."),
  ) + 
  scale_fill_brewer(type = "qual", palette = "Set1") + 
  scale_y_continuous(breaks = seq(0, 35, by = 5), limits = c(0, 32)) + 
  theme(legend.position = "none")

```

```{r}
top_issues_overall <- mpeds_police_issue$value[1:10]
```

```{r police_issue_baseline, width = 10, height = 8, include = FALSE}
# Where all_protests is "0", and we just graph the diffs above/below the x-axis for the
# top X overall protests

all_cats <- expand_grid(
  value = top_issues_overall, 
  type = c("Police-involved protests", "Protests with extreme policing")
)

police_issue_combined |> 
  filter(value %in% top_issues_overall,
         type != "All protests") |> 
  right_join(all_cats, by = c("type", "value")) |> 
  left_join(police_issue_diffs, by = c("type", "value")) |> 
  mutate(
    value = factor(value, levels = top_issues_overall),
    value = fct_recode(value, 
      `University admin` = "University governance, admin, policies, programs, curriculum",
      `University admin (racial)` = "University governance, admin, policies, programs, curriculum (racial)",
      `Anti-racism` = "Anti-racism (racial)",
      `Anti-Trump` = "Trump and/or his administration (Against)",
      `Pro-immigrants` = "Immigration (For) (racial)",
      `Racialized police violence` = "Police violence (racial)",
      `Other issue` = "_Other Issue"
    ) |> fct_relabel(~str_wrap(., 13, whitespace_only = FALSE)), 
    diff_value = ifelse(is.na(diff_value), 0 - all_protests, diff_value),
    diff = ifelse(is.na(diff), 
                  ifelse(diff_value < 0, paste0("-", round(diff_value, 2)),
                         paste0("+", round(diff_value, 2))),
                  diff),
    type = factor(type, levels = c("Protests with extreme policing", "Police-involved protests"))
  ) |> 
  ggplot() + 
  ...
  geom_text(aes(x = value, 
                group = type,
                y = ifelse(diff_value < 0, diff_value - 1.5, diff_value + 1.5),
                label = diff),
            family = "Lato",
  ...
  labs(
    fill = NULL,
    x = NULL,
    y = "Difference from issue prevalence for all protests"
  ) +
  scale_fill_brewer(type = "qual", palette = "Set1") + 
...
```{r police_involvement_country}

police_involvement |> 
  filter(!str_detect(key, "^Umbrella")) |> 
  select(country, issue, racial_issue) |> 
  pivot_longer(cols = c(issue, racial_issue)) |> 
  unnest(cols = value) |> 
  filter(value != "_Not relevant") |> 
  drop_na() |> 
...
police_involvement |> 
  filter(!str_detect(key, "^Umbrella")) |> 
  mutate(is_public = if_else(map_lgl(university, ~.$is_uni_public[1]), "Public", "Private")) |> 
  select(is_public, issue, racial_issue) |> 
  pivot_longer(cols = c(issue, racial_issue)) |> 
  unnest(cols = value) |> 
  filter(value != "_Not relevant") |> 
  drop_na() |> 
  ...
  select(-name) |> 
  group_by(is_public, value) |> 
  count() |> 
  ungroup() |> 
  pivot_wider(names_from = is_public, values_from = n) |> 
  select(issue = value, Public, Private) |> 
  arrange(desc(Public)) |> 
  kable()
...
make_line_plot <- function(dta){
  ...
  ggplot(data = dta) +
    ...
    labs(
      ...
      caption = str_wrap("Dates are aggregated to months.")
    ) 
}
line_dta <- mpeds |> 
  mutate(date = floor_date(start_date, "month"),
         country = if_else(str_extract(geoid, "us|canada") == "us", "US", "Canada")
         ) |> 
  filter(!is.na(country)) 

events <- tribble(
  ~date, ~x_offset, ~text, ~country, ~height, 
  "2012-02-01", NA, "Quebec\nprotests", "Canada", 300,
  "2014-11-01", -180, "Darren Wilson\nnon-indictment\nprotests", "US", 275,
  "2015-10-01", -120, "Mizzou\nprotests", "US", 325,
  "2015-11-01", 150, "Toronto labor\nprotests", "Canada", 275,
  "2016-11-01", 150,"Anti-Trump\nprotests", "US", 300
) |> 
  mutate(date = as.Date(date),
         x_offset = if_else(is.na(x_offset), 120, x_offset))
...
line_dta |> 
  ...
  geom_vline(aes(xintercept = as.Date(date), color = country), data = events,
            linetype = "dashed") + 
  geom_label(
    data = events,
    aes(x = as.Date(date) + x_offset, y = height, label = text, color = country),
    family = "Lato") + 
  ...
  coord_cartesian(expand = FALSE, ylim = c(0, 400),
                  xlim = c(as.Date("2011-12-02"), as.Date("2018-11-30"))) +
  theme(legend.position = "none")
...
line_dta |> 
  ...
  drop_na() |> 
  make_line_plot() + 
  geom_line(aes(x = date, y = n, color = country)) +
  geom_vline(aes(xintercept = as.Date(date), color = country), data = events, linetype = "dashed") + 
  geom_label(
    data = events,
    aes(x = as.Date(date) + x_offset, y = height, label = text, color = country),
    family = "Lato") +
...
events_mod <- events |> 
  mutate(height = if_else(country == "Canada", 100, height - 50),
         x_offset = if_else(country == "Canada", x_offset + 40, x_offset)) 

line_dta |> 
  group_by(date, country) |> 
  count() |> 
  group_by(country) |> 
  mutate(max_n = max(n, na.rm = TRUE)) |> 
  ungroup() |> 
  drop_na() |> 
  make_line_plot() + 
  geom_line(aes(x = date, y = n, color = country)) +
  geom_vline(aes(xintercept = as.Date(date), color = country),
             data = events_mod, linetype = "dashed") + 
  geom_label(
    data = events_mod,
    aes(x = as.Date(date) + x_offset,
        y = height, label = text, color = country),
    family = "Lato") +
  scale_color_manual(values = c(
    "US" = "blue",
    "Canada" = "red"
  ...
  coord_cartesian(expand = FALSE,
                  ylim = c(0, NA),
                  xlim = c(as.Date("2011-12-02"), as.Date("2018-11-30"))) +
  geom_blank(aes(y = max_n + 50, color = country)) + 
...
title_grob <- ggdraw() + 
  draw_label("Counts of protest events over time",
             x = 0.05, 
             hjust = 0, 
             fontfamily = "Lato",
             fontface = "bold"
...
protests_region_plot <- ggplot(protests_by_region,
  ...
  labs(
    ...
    y = "Number of associated protest events", 
...
title_grob <- ggdraw() + 
  draw_label("Counts of protest events over time by region",
             x = 0.05, 
             hjust = 0, 
             fontfamily = "Lato",
             fontface = "bold"
...
ggplot(protests_by_country, aes(fill = country, x = date, y = n)) + 
  ...
  labs(
    ...
    x = "Date (floored to months)",
    fill = "Country",
    title = "Protest events by country"
  ) 
```
...
get_uni_protest_took_place <- function(uni_df){
  uni <- uni_df |> 
    filter(
      uni_name_source %in% c("publication", "other univ where protest occurs")
    ) |> 
    arrange(uni_name_source) |> 
    slice_head(n = 1) |> 
    pull(university_name)
  return(ifelse(length(uni) > 0, uni, NA_character_))
}

unis_over_time <- mpeds |> 
  mutate(date = floor_date(start_date, "month"),
         university_took_place = map_chr(
           university, get_uni_protest_took_place
           )) |> 
  group_by(university_took_place, date) |> 
  count() |> 
  drop_na() |> 
  filter(university_took_place %in% university_counts$university_name[1:10]) |> 
  mutate(university_took_place = str_wrap(university_took_place, 15)) |> 
  left_join(uni_grouping, by = c("university_took_place" = "university"))

uni_plot <- ggplot(unis_over_time,
                   aes(x = date,
                       y = n, 
                       fill = university_took_place)) + 
  geom_bar(stat = "identity") + 
...
uni_grouped_plot <- ggplot(uni_grouping_over_time,
                           aes(x = date, y = n, fill = group)) + 
  geom_bar(stat = "identity") + 
  scale_fill_brewer(type = "qual", palette = "Set1") + 
  scale_x_date(breaks = "4 months",
               date_labels =  "%b",
...
plot_response_over_time <- function(colname){
  ...
  unnested |> 
    ...
    labs(
      title = paste("Top 9 ", name_string, "categories over time"),
...
police_activity_xwalk <- list(
  ...
  "Passive Control" = c(
    "Cooperate/Coordinate", "Monitor/Present", "Participate", "Present"
  ),
  "Verbal Communication" = c(
    "Instruct/Warn", '"Breaking the Rules"', "Formal Accusation",
    "Disputed Actions", "\"We're Responsive\""
  ),
  ...
  lmap(\(list_item){
    ...
    return(tibble(umbrella = name, categories = unlist(set_names(list_item, NULL))))
  }) |> 
  reduce(bind_rows)

police_activity_xwalk |> 
  rename("New umbrella category" = umbrella,
         "Existing police activity" = categories) |> 
  kable()

mpeds |> 
  filter(start_date < "2018-06-01") |> 
  mutate(date = floor_date(start_date, "month")) |> 
  select(date, police_activities) |> 
  unnest(police_activities) |> 
  left_join(police_activity_xwalk, by = c("police_activities" = "categories")) |> 
  count(date, umbrella) |> 
  filter(umbrella != "NA/Unclear") |> 
  ggplot() + 
  geom_bar(aes(x = date, y = n, fill = umbrella),
           stat = "identity") + 
  scale_fill_brewer(type = "qual", palette = "Set1") + 
  ...
  labs(
    y = "Number of associated events",
    x = NULL,
    title = "Police activities over time",
    fill = "Type of police activity"
  )
...
mpeds |>   
  ...
  geom_bar(stat = "identity") +
  labs(
    y = "Share of protest events during given month",
    x = "Date (floored to months)",
...
We're also interested in understanding if the biggest upticks in protest counts
are driven by an uptick in racial issues. This is

```{r}

# scatter plot of prop. racial vs total n of events
mpeds_issue_time <- mpeds |> 
  mutate(date = floor_date(start_date, "month"),
         is_racial = map_lgl(racial_issue, has_issue),
         is_issue = map_lgl(issue, has_issue)) |> 
  group_by(date) |> 
  summarize(
    n_total = n(),
    n_racial = sum(is_racial, na.rm = T),
    n_issue = sum(is_issue, na.rm = T)
  ) |> 
  drop_na()

mpeds_issue_time |> 
  ggplot(aes(x = n_racial/n_total, y = n_total)) + 
  geom_point(alpha = 0.5) + 
  geom_smooth() +
  labs(
    y = "Total number of events in month",
    x = "Proportion of events with racial issues",
    title = "Are increases in event count driven by upticks in racial issues?",
  ) 

mpeds_issue_time |> 
  ggplot(aes(x = date, y = n_total, fill = 100 * n_racial/n_total)) + 
  geom_bar(stat = "identity") + 
  labs(
    title = "Are increases in event count driven by upticks in racial issues?",
...
```{r, width = 16, height = 14}
# For racial issue and issue separately: take top ten categories, 

top_issues_combined <- mpeds |> 
  select(issue, racial_issue) |> 
...
issue_heatmap <- mpeds |> 
  mutate(
    y = map2(issue, racial_issue, \(x, y){
      c(x[x != "_Not relevant"], y) |> 
        str_wrap(width = 10)
      }),
    x = y) |> 
  select(country, x, y) |> 
  ...
  count(country, x, y) |> 
  mutate(n = if_else(x == y, Inf, n))

get_issue_cooccurrence <- function(country_name){
  issue_export <- issue_heatmap |> 
    filter(country == country_name) |> 
    pivot_wider(names_from = x, values_from = n) |> 
    mutate(across(where(is.numeric), ~if_else(is.na(.), 0, .)),
           across(where(is.numeric), ~if_else(is.infinite(.), NA, .))) |> 
    arrange(y)
  issue_top_export <- issue_heatmap |> 
    filter(country == country_name) |> 
    filter(x %in% top_issues_combined,
           y %in% top_issues_combined) |> 
    pivot_wider(names_from = x, values_from = n) |> 
    mutate(across(where(is.numeric), ~if_else(is.na(.), 0, .)),
           across(where(is.numeric), ~if_else(is.infinite(.), NA, .))) |> 
    arrange(y)
  list(issue_export, 
       issue_top_export) |> 
    set_names(paste0(country_name, "_", c("all_issues", "top_issues")))
}
# writexl::write_xlsx(list(get_issue_cooccurrence("US"), get_issue_cooccurrence("Canada")) |> 
#                       flatten(), "~/Downloads/issues.xlsx")

make_issue_heatmap <- function(country_name) {
  issue_heatmap |> 
    mutate(n = if_else(is.infinite(n), NA, n)) |> 
    filter(country == country_name, 
           x != y, 
           x %in% top_issues_combined,
           y %in% top_issues_combined) |> 
    ggplot(aes(x, y, fill = n)) + 
    geom_tile() + 
    labs(title = paste0("Issue co-occurrence in ", country_name)) +
    theme(
    ...
    scale_fill_viridis(option = "magma", begin = 0.1)
}
make_issue_heatmap("US")
make_issue_heatmap("Canada")
...
mpeds |> 
  ...
  labs(
    title = "Glimpse of numeric covariates at the protest event level",
    y = "Number of occurrences",
    x = "Value of variable",
  )
...
mpeds_sf |> 
  ...
  labs(
    title = "Spread of protest events and geocoded locations",
...
mizzou_ids <- mizzou_events$canonical_id1
...
ggplot(mizzou_sf) +
  ...
  geom_sf(aes(size = n, shape = type, color = type), alpha = 0.8) + 
  labs(
    title = "Spread of Mizzou anti-racism protests and solidarity protests, fall 2015- spring 2016"
  ) + 
  ...
  scale_size_continuous(breaks = seq(1, 7, by = 2), range = c(2, 8)) + 
  guides(shape = FALSE,
         color = guide_legend(override.aes = list(size = 4)),
         size = guide_legend(override.aes = list(color = "#FA918A")))

tibble(canonical_id = as.character(mizzou_ids)) |> 
  ...
  labs(
    x = "Date",
    y = "Number of protest events starting on date",
...
tibble(canonical_id = as.character(mizzou_ids)) |>
  ...
  filter(value != "_Not relevant") |> 
  unnest(value) |> 
  mutate(value = ifelse(name == "racial_issue", paste0(value, " (racial)"), value)) |> 
  group_by(value) |> 
  count() |> 
  arrange(desc(n)) |> 
  kable()
...
antiracism_issues <- c(
  ...
  "Hate crimes/Anti-minority violence",
  "Hate speech",
  "Immigration (For)",
  "Indigenous issues",
  "Memorials & anniversaries",
  "Racist/racialized symbols",
  "Reparations",
  "University governance, admin, policies, programs, curriculum",
  "White supremacy (Against)"
)

antiracism_metadata <- mpeds |>
  mutate(is_issue_unigovernance = map_lgl(racial_issue, ~any(
           . == "University governance, admin, policies, programs, curriculum"
         )),
         is_issue_campusclimate = map_lgl(racial_issue, ~any(
           . == "Campus climate"
         )),
         is_target_individual = map_lgl(target, ~("Individual" %in% .)),
         is_target_university = map_lgl(target, ~("University/school" %in% .))) |>
  filter(map_lgl(racial_issue, ~any(antiracism_issues %in% .))) |> 
  select(start_date,
         key,
         university,
         is_issue_unigovernance,
         is_issue_campusclimate,
         is_target_individual,
         is_target_university) 

# Below is used to manually inspect 2015 events
antiracism_2015 <- antiracism_metadata |>
  filter(floor_date(start_date, "month") == "2015-11-01") |> 
  select(-start_date)

# Produce a summary and compare it to other November protests
antiracism_metadata |>
  mutate(year = year(start_date)) |>
  filter(month(start_date) == "11",
         year != 2018) |>
  drop_na() |>
  group_by(year) |>
  summarize(across(
    c(is_issue_unigovernance,
      is_issue_campusclimate,
      is_target_individual,
      is_target_university),
    sum),
    is_antiracism = n()) |> 
  pivot_longer(cols = c(is_issue_unigovernance,
      is_issue_campusclimate,
      is_target_individual,
      is_target_university
      )) |> 
  mutate(name = case_when(
    name == "is_issue_unigovernance" ~ "Issue relates to university governance",
    name == "is_issue_campusclimate" ~ "Issue relates to campus climate",
    name == "is_target_individual" ~ "Target is individual",
    name == "is_target_university" ~ "Target is university",
    TRUE ~ name
  ...
  geom_text(aes(label = value),
            ...
            vjust = -1) +
  labs(
    title = "Comparison of November Anti-racism Protests",
    y = "Number of associated protest events",
    x = NULL,
    fill = NULL
  )  + 
...
```{r mizzou_articles}
events <- c(
  "Umbrella_Mizzou_Anti-Racism_2015_Oct-Nov",
  "20151001_Columbia_March_AntiRacism",
  "20151006_Columbia_Sit-in_AntiRacism",
  "20151010_Columbia_Blockade_UniversityGovernance",
  "20151021_Columbia_OtherForm_UniversityGovernance",
  "20151102_Columbia_HungerStrike_UniversityGovernance",
  "20151102_Columbia_Occupation_UniversityGovernance",
  "20151107_Columbia_Boycott_UniversityGovernance",
  "20151107_Columbia_Demonstration_CampusClimate",
  "20151109_Columbia_Rally_CampusClimate",
  "20151109_Columbia_OtherForm_FreeSpeech",
  "20151110_Columbia_FacultyWalkout_UniversityGovernance"
)

articles <- tar_read(articles)
articles |> 
  mutate(canonical_key = str_trim(canonical_key)) |> 
  filter(canonical_key %in% events) |> 
  group_by(canonical_key) |> 
  summarize(
    `Article Mentions` = length(unique(article_id)),
    `Newspaper Mentions` = length(unique(publication)),
  ) |> 
  arrange(desc(`Article Mentions`)) |> 
  kable()
...
get_mizzou_responses <- function(colname){
  ...
  mpeds |> 
    ...
    rename({{name_string}} := {{colname}},
           "Number of associated protest events" = n) |> 
    kable()
}
# 
...
quebec_sf <- tibble(canonical_id = as.character(quebec_ids)) |> 
...
tibble(canonical_id = as.character(quebec_ids)) |> 
  left_join(mpeds, by = "canonical_id") |> 
...
plot_quebec_by_police <- function(colname,
  ...
  tibble(canonical_id = as.character(quebec_ids)) |> 
    ...
    mutate(umbrella = ifelse(is.na(umbrella), categories, umbrella),
           umbrella = case_when(
             nrow(xwalk) > 0 ~ factor(umbrella, unique(xwalk$umbrella)),
             TRUE ~ as.factor(umbrella)
           )) |> 
...
```{r quebec_extreme_police}

# grouped bar graph: fill = non-extreme and extreme police involvement, group = quebec vs all events
quebec_events <- tibble(canonical_id = as.character(quebec_ids)) |> 
  left_join(mpeds, by = "canonical_id")

quebec_extreme_police <- lmap(list("Quebec" = quebec_events, "All" = mpeds), \(list_item){
  denominator <- list_item[[1]] |> 
    pull(key) |>
    unique() |>
    str_subset("^Umbrella", negate = TRUE) |> 
    length()
  counted <- list_item[[1]] |>
    mutate(
      `Extreme Police Involvement` = (
        map_lgl(police_activities, ~"Force: 2+ Weapon Types" %in% .) +
          map_lgl(police_activities, ~"Arrest- Large Scale" %in% .) + 
          map_lgl(police_presence_and_size, ~"Heavily Policed" %in% .) +
          map_lgl(police_presence_and_size, ~"Motorized Presence" %in% .) +
          map_lgl(police_presence_and_size, ~"Substantial" %in% .) >= 2) |
        map_lgl(type_of_police, ~'"Riot police"' %in% .),
      `Any Police Involvement` = if_any(c(type_of_police, police_actions_text,
                                 police_activities, police_presence_and_size),
                               ~map_lgl(., \(x){!is.null(x)})) &
           if_any(c(type_of_police, police_actions_text,
                    police_activities, police_presence_and_size),
    ...
    pivot_longer(cols = c(`Extreme Police Involvement`, `Any Police Involvement`)) |> 
    mutate(date = floor_date(start_date, "week")) |> 
    group_by(date, name) |> 
    summarize(value = sum(value),
              n = denominator)
  list(counted) |> set_names(names(list_item))
}) |> 
  bind_rows(.id = "type")

dates <- quebec_extreme_police |> 
  filter(type == "Quebec") |> 
  drop_na(date) |> 
  pull(date)
date_xwalk <- expand_grid(
  date = seq(min(dates), max(dates), by = "1 week"),
  type = "Quebec",
  name = c("Any Police Involvement", "Extreme Police Involvement")
)

quebec_extreme_police |> 
  filter(type == "Quebec") |> 
  right_join(date_xwalk, by = c("date", "type", "name")) |> 
  mutate(value = ifelse(is.na(value), 0, value),
         value = ifelse(name == "Any Police Involvement", value + 0.04, value),
         type = ifelse(is.na(type), "Quebec", type)) |> 
  drop_na(name) |> 
  ggplot(aes(x = date, y = value, color = str_wrap(name, 15))) + 
  geom_line(size = 0.6) + 
  scale_x_date(date_breaks = "1 month",
               date_labels = "%b") +
  scale_color_brewer(type = "qual", palette = "Set1") +
  labs(color = NULL, y = "Number of associated protest events",
       x = NULL, title = "Quebec protests with general and extreme levels of police involvement") +
  guides(color = guide_legend(byrow = TRUE)) 

quebec_extreme_police |> 
  group_by(type, name) |> 
  summarize(value = 100 * sum(value) / n,
            n = unique(n)) |> 
  distinct() |> 
  mutate(type = paste0(type, " (n=", n, ")")) |> 
  ggplot(aes(x = type, fill = str_wrap(name, 15), y = value)) + 
  geom_bar(position = position_dodge(), stat = "identity") +
  scale_fill_brewer(type = "qual", palette = "Set1") +
  labs(fill = NULL, x = NULL, y = "Percentage of all protests", 
       title = "Police involvement in Quebec protests and all protests")  + 
  geom_text(aes(label = paste0(round(value, 2), "%")),
            family = "Lato",
            vjust = -1, position = position_dodge(0.9)) +
  scale_y_continuous(breaks = seq(0, 45, by = 5), limits = c(0, 42)) + 
  guides(fill = guide_legend(byrow = TRUE)) + 
...
ggplot(trump) + 
  geom_sf(data = canada_province_shapes, color = NA) + 
  geom_sf(data = us_sf) + 
  geom_sf(aes(size = n, color = issue)) + 
  lims(
  x = c(-125, -65), 
  y = c(24, 50)
  ) + 
  labs(
    size = "Number of events",
    title = "Geographic spread of Trump-related protests",
    color = NULL
  ) +
  scale_color_manual(
    values = c(
