...
opts_chunk$set(echo = FALSE, message = FALSE, fig.width = 12, fig.height = 8)
...
unis_over_time <- mpeds |> 
  mutate(start_date = lubridate::floor_date(start_date, "month")) |> 
  group_by(university, start_date) |> 
  count() |> 
  drop_na() |> 
  filter(university %in% university_counts$university[1:10]) 

ggplot(unis_over_time, aes(x = start_date, y = n, color = university)) + 
  geom_line() + 
  labs(
    x = "Year",
    y = "Number of associated canonical events",
    title = "Top universities by protest count over time",
    caption = "Universities displayed are top 10 overall universities
...
```{r regions_over_time}
regions_over_time <- mpeds |> 
  mutate(start_date = lubridate::floor_date(start_date, "month"),
         region = case_when(
           !is.na(canada_metropolitan_area) ~ "Canada",
           is.na(region) ~ NA_character_,
           TRUE ~ region)
         ) |> 
  group_by(region, start_date) |> 
  count() |> 
  drop_na()

ggplot(regions_over_time,
       aes(color = region, x = start_date, y = n)) + 
  geom_line() +
  labs(
    y = "Number of associated canonical events",
    x = "Month of protest",
    title = "Canonical events over time by geographical region",
...
```{r binned_summary_by_uni}
counts_quintile <- mpeds |>
  group_by(university) |> 
  count() |> 
  ungroup() |> 
  mutate(n = ntile(n, 5)) |> 
...
mpeds |> 
  select(university, where(function(x){is.numeric(x) || is.logical(x)}),
                 -canonical_id, -starts_with("location"),
         -bachelors_granting, -phd_granting, -masters_granting,
                 -year, -uni_id, -size_category, -link) |> 
  left_join(counts_quintile, by = "university") |> 
  filter(n == 5 | n == 1) |> 
  pivot_longer(cols = c(-university, -n)) |> 
  filter(name != "NA") |> 
  group_by(n, name) |> 
  summarize(
    type = ifelse(is.numeric(pull(mpeds[name[1]], 1)), "numeric", "boolean"),
