...
n_uni_police_only <- mpeds |> 
  ...
  mutate(x  = map_lgl(type_of_police, function(x){
...
n_govt_police_only <- mpeds |> 
  filter(!map_lgl(type_of_police, is.null)) |> 
  filter(map_lgl(type_of_police, function(x){
    all(x %in% c("Govt police", "Govt police - assumed"))
  })) |> 
  nrow()
...
tribble(
  ...
  "# of events with police presence recorded", n_has_police,
...
protests_by_country <- mpeds |> 
  mutate(
    country = ifelse(
      is.na(canada_metropolitan_area), "United States", "Canada"
    )
    ) |> 
  mutate(date = floor_date(start_date, "month")) |> 
...
tar_load(canada_cma_shapes)

canada_cma_shapes <- st_set_crs(canada_cma_shapes, st_crs(county_sf)) |> 
  select(canada_metropolitan_area)
...
mizzou_sf <- mizzou_events |> 
  ...
  slice_head(n = 1) |> 
  group_by(fips, canada_metropolitan_area) |> 
  count() |> 
  ungroup() |> 
  left_join(county_sf, by = "fips") |> 
  mutate(county_geo = st_centroid(geometry)) |> 
  select(-geometry) |> 
  left_join(canada_cma_shapes, by = "canada_metropolitan_area") |> 
  mutate(geometry = ifelse(is.na(canada_metropolitan_area),
                           county_geo,
...
quebec_sf <- quebec_events |> 
  ...
  mutate(place = ifelse(!is.na(canada_metropolitan_area), 
                        canada_metropolitan_area,
                        name)) |> 
  group_by(place) |> 
  count() |> 
  ungroup() 
...
```{r}
kable(st_drop_geometry(top_quebec))
```
