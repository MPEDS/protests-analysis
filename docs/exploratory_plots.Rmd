...
police_involvement_issue <- police_involvement |> 
  ...
  count() |> 
  ungroup() |> 
  mutate(n = round(100 * n/length(unique(police_involvement$canonical_id)), 2)) |> 
  arrange(desc(n)) 
...
mpeds_police_issue <- mpeds |> 
  ...
  filter(value %in% top_police_issues$value) |> 
  select(-name) |> 
  group_by(value) |> 
  count() |> 
  ungroup() |> 
  mutate(n = round(100 * n/length(unique(mpeds$canonical_id)), 2)) 

list("All protests" = mpeds_police_issue,
     "Police-involved protests" = top_police_issues) |> 
  bind_rows(.id = "type") |> 
  mutate(value = str_wrap(value, 12),
         value = factor(value, levels = str_wrap(top_police_issues$value, 12))) |> 
  ggplot(aes(x = value, fill = value, y = n)) + 
  geom_bar(stat = "identity") + 
  facet_wrap(~type, nrow = 2) + 
  labs(
    title = "Top issue occurrences in police-involved and general protests",
    y = "Percentage of protests associated with issues",
    x = NULL,
    caption = "Top issues are defined as the seven most common issues for police-involved protests."
  ) + 
...
plot_quebec_by_police <- function(colname){
  ...
  tibble(canonical_id = quebec_ids) |> 
    ...
    labs(
      ...
      title = paste0(name_string, " during 2012 Quebec Tuition Strike"),
      fill = name_string
    )
}

plot_quebec_by_police(police_presence_and_size) 
plot_quebec_by_police(police_activities) 
plot_quebec_by_police(type_of_police) 
```

Brainstorming a breakdown for activity recategorization:

- Constrain: Arrest or Attempted, Arrest- Large Scale, Constrain, Detain,   
