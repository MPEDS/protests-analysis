...
buckets <- tribble(
  ~bucket, ~issue,
  "Support for Racially Marginalized Students", "Campus climate",
  "Support for Racially Marginalized Students", "Affirmative action (For)",
)
...
make_issue_counts <- function(country_opt, sub_dta = NULL){
  ...
  with_buckets <- bucket_counts |> 
    mutate(bucket_placeholder = value) |> 
    bind_rows(raw) |> 
    left_join(buckets, by = c("value" = "issue")) |> 
    mutate(n = ifelse(is.na(total_n), n, total_n)) |> 
    select(-total_n) |> 
    left_join(bucket_counts, by = c("bucket" = "value")) |> 
    mutate(total_n = ifelse(is.na(total_n), n, total_n), 
           name = ifelse(is.na(name), "total", name), 
           bucket = ifelse(is.na(bucket_placeholder), bucket, bucket_placeholder)) |> 
    arrange(desc(total_n), bucket) |> 
    select(-bucket_placeholder, -total_n) 
  ...
  formatted <- with_buckets |> 
    ...
    mutate(
      ...
      bucket = ifelse(is.na(bucket), "", bucket),
      across(
        where(is.numeric), ~ifelse(
          is.na(.), "", paste0(round(100 * ./total_n, 2), "% (", ., ")"))
      )) |> 
